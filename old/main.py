...
def go_eat():
    ...
    try:
        ...
        driver.request('POST', login_data['ajax_url'], data = {'mod': 'inventory', 'submod': 'move', 'from': food_data['bag'], 'fromX': res.text[i2], 'fromY': res.text[i3], 'to': 8, 'toX': 1, 'toY': 1, 'amount': 1, 'doll': 1, 'a': get_milliseconds(), 'sh': secureHash})
        ...
        for pos in posi_sequence:
            if res.text.find(f"startFight('{pos}', '{dungeonID}')") != -1: # Check if the button is present, so we don't spam pointless requests
                driver.request('GET', f"https://s{login_data['server_number']}-{login_data['server_country']}.gladiatus.gameforge.com/game/ajax/doDungeonFight.php",
                                      data = {'did': dungeonID, 'posi': pos, 'a': get_milliseconds(), 'sh': secureHash})
                break
...
def go_expedition(location, stage):
    ...
    try:
        res = driver.request('GET', login_data['ajax_url'], data = {'mod': 'location', 'submod': 'attack', 'location': location, 'stage': stage, 'premium': 0, 'a': get_milliseconds(), 'sh': secureHash})
    except Exception as e:
        ...
        try:
            for x in range(5):
                ...
                if nivel_oponent < max_fight_level:
                    ...
                    driver.request('GET', login_data['ajax_url'], data = {'mod': 'arena', 'submod': 'doCombat', 'aType': 2, 'opponentId': id_player, 'serverId': id_server, 'country': login_data["country"], 'a': get_milliseconds(), 'sh': secureHash})
...
def plan_manager():
    ...
    while True:
        ...
        driver.request('POST', login_data['ajax_url'], data = {'mod': 'location', 'submod': 'attack', 'location': 'nile_bank', 'stage': 1, 'premium': 0, 'a': get_milliseconds(), 'sh': secureHash})
